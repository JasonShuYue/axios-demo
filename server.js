...
var server = http.createServer(function(request, response){
  ...
  //  response.write('123;body{background-color: red;}h1{color:blue;}')
  //  response.end()
  ...
  // }
    ...
    if(path === "/") {
      let string = fs.readFileSync("./dist/index.html", "utf-8");
      response.setHeader("Content-Type", "text/html; charset=utf-8;");
      response.statusCode = 200;
      response.write(string);
      response.end();
    } else if(path === "/css/style.css"){
        let string = fs.readFileSync("./dist/css/style.css", "utf-8");
        response.setHeader("Content-Type", "text/css; charset=utf-8;");
        response.statusCode = 200;
        response.write(string);
        response.end();
    } else if(path === "/js/main.js") {
        let string = fs.readFileSync("./dist/js/main.js", "utf-8");
